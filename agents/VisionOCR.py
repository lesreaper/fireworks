...
class VisionOCRAgent(BaseAgent):
    ...
    def _extract_text_from_response(self, response: Dict) -> tuple[str, float]:
        ...
        if self.api_key:
            ...
            if "fullTextAnnotation" in result:
                ...
                confidence = result.get("confidence", 0.95)  # Default high confidence for text detection
            else:
                return "", 0.0
        else:
            # Client library response
            if hasattr(response, 'full_text_annotation') and response.full_text_annotation:
                ...
                confidence = 0.95  # Default high confidence for text detection
            else:
                return "", 0.0
    ...
    def process(self, state: State) -> State:
        ...
        try:
            ...
            # Use document text if available, fall back to regular text
            ...
            state.update({
                ...
                "image_data": final_text  # Add text to image_data for compatibility with existing agents
            })
